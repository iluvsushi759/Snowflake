-- 04_data_quality_checks.sql
-- ✅ Row counts
SELECT
    'PATIENTS' AS TABLE_NAME,
    COUNT(*) AS ROW_COUNT
FROM ${TARGET_SCHEMA}.PATIENTS;

SELECT
    'TRIALS' AS TABLE_NAME,
    COUNT(*) AS ROW_COUNT
FROM ${TARGET_SCHEMA}.TRIALS;

SELECT
    'PARTICIPATION' AS TABLE_NAME,
    COUNT(*) AS ROW_COUNT
FROM ${TARGET_SCHEMA}.PARTICIPATION;

-- ✅ Referential integrity: PARTICIPATION → PATIENTS
SELECT COUNT(*) AS INVALID_PATIENT_REFS
FROM ${TARGET_SCHEMA}.PARTICIPATION AS PT
LEFT JOIN ${TARGET_SCHEMA}.PATIENTS AS P ON PT.PATIENT_ID = P.PATIENT_ID
WHERE P.PATIENT_ID IS NULL;

-- ✅ Referential integrity: PARTICIPATION → TRIALS
SELECT COUNT(*) AS INVALID_TRIAL_REFS
FROM ${TARGET_SCHEMA}.PARTICIPATION AS PT
LEFT JOIN ${TARGET_SCHEMA}.TRIALS AS T ON PT.TRIAL_ID = T.TRIAL_ID
WHERE T.TRIAL_ID IS NULL;

-- ✅ Null checks in critical fields
SELECT COUNT(*) AS NULL_PATIENT_NAMES
FROM ${TARGET_SCHEMA}.PATIENTS
WHERE FIRST_NAME IS NULL OR LAST_NAME IS NULL;

SELECT COUNT(*) AS NULL_TRIAL_NAMES
FROM ${TARGET_SCHEMA}.TRIALS
WHERE TRIAL_NAME IS NULL;

SELECT
    COUNT(*) AS NULL_PARTICIPATION_STATUS
FROM ${TARGET_SCHEMA}.PARTICIPATION
WHERE PARTICIPATION_STATUS IS NULL;

-- ✅ Optional: Consent date after DOB
SELECT COUNT(*) AS INVALID_CONSENT_DATES
FROM ${TARGET_SCHEMA}.PARTICIPATION AS PT
JOIN ${TARGET_SCHEMA}.PATIENTS AS P ON PT.PATIENT_ID = P.PATIENT_ID
WHERE PT.CONSENT_DATE < P.DOB;
